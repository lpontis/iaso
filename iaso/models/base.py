...
class Task(models.Model):
    ...
    def report_progress_and_stop_if_killed(
        self, progress_value=None, progress_message=None, end_value=None
    ):
...
class Instance(models.Model):
    ...
    def as_full_model(self):
        ...
        return {
            ...
            "created_at": self.created_at.timestamp() if self.created_at else None,
            "updated_at": self.updated_at.timestamp() if self.updated_at else None,
            "org_unit": self.org_unit.as_dict_with_parents(light=False, light_parents=False)
            if self.org_unit
            else None,
            "latitude": self.location.y if self.location else None,
            "longitude": self.location.x if self.location else None,
            ...
            "file_content": file_content,
            "files": [
                f.file.url if f.file else None for f in self.instancefile_set.filter(deleted=False)
            ],
            "status": getattr(self, "status", None),
            "correlation_id": self.correlation_id,
...
class Profile(models.Model):
    ...
    def as_dict(self):
        return {
            ...
            "permissions": list(
                self.user.user_permissions.filter(codename__startswith="iaso_").values_list(
                    "codename", flat=True
                )
            ),
