...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--version_number_ref", type=int, help="An integer version number for the new version"
        )
        parser.add_argument(
            "--validation_status_ref", type=str, help="Validation status ref", required=False
        )
        parser.add_argument(
            "--top_org_unit_ref", type=int, help="Top org unit ref id", required=False
        )
        parser.add_argument("--top_org_unit", type=int, help="Top org unit id", required=False)
        parser.add_argument(
            "--org_unit_types",
            type=str,
            help="Org unit types ids, separated by a comma",
            required=False,
        )
        parser.add_argument(
            "--org_unit_types_ref",
            type=str,
            help="Org unit types ids, separated by a comma, for ref",
            required=False,
        )

        parser.add_argument(
            "--export", action="store_true", help="really export to dhis2 or only dry run"
        )
                ...
                (
                    ...
                    ", ".join(
                        list(
                            map(
                                lambda x: str(x),
                                SourceVersion.objects.filter(data_source=source).values_list(
                                    "number", flat=True
                                ),
                            )
                        )
