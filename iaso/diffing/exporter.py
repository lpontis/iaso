    ...
    def update_groups(self, api, diffs, fields):
        ...
        for groupset_field_type in groupset_field_types:
            self.iaso_logger.info(
                "---", groupset_field_type.groupset_ref, groupset_field_type.groupset_name
            )
                    ...
                    if comparison.status == "new" or comparison.status == "modified":
                        ...
                        if len(tokeep) > 0:
                            if not dhis2_group_contains(dhis2_group, diff.org_unit):
                                ...
                                modified = True
                                self.iaso_logger.info(
                                    "\t added : ", diff.org_unit.name, diff.org_unit.source_ref
                                )
                        else:
                            if dhis2_group_contains(dhis2_group, diff.org_unit):
                                ...
                                modified = True
                                self.iaso_logger.info(
                                    "\t removed : ", diff.org_unit.name, diff.org_unit.id
                                )
                if modified:
                    ...
                    self.iaso_logger.info(
                        "updated  ", dhis2_group["id"], dhis2_group["name"], resp, resp.json()
                    )
        return
