...
class AppsAPITestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.yoda = cls.create_user_with_profile(
            username="yoda", account=account, permissions=["iaso_forms"]
        )
        cls.project_1 = m.Project.objects.create(
            name="Project 1", account=account, app_id="org.ghi.p1"
        )
        cls.project_2 = m.Project.objects.create(
            name="Project 2", account=account, app_id="org.ghi.p2"
        )
        cls.flag_1 = m.FeatureFlag.objects.create(
            code="send_location",
            name="Send GPS location",
            description="Send GPS location every time etc",
        )
    ...
    def test_app_create_ok_with_auth(self):
        candidated_app = {
            ...
            "feature_flags": [
                {"id": self.flag_1.id, "name": self.flag_1.name, "code": self.flag_1.code}
            ],
    ...
    def test_app_create_auto_commit_require_auth_ok_with_auth(self):
        ...
        response_data = response.json()
        self.assertTrue(
            "REQUIRE_AUTHENTICATION" in list(ff["code"] for ff in response_data["feature_flags"])
        )
